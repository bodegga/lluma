...
- **R5 — keep the traits in `lluma-issuer`; durable impls FAIL CLOSED on IO error (CONTROLLER DEVIATION from "make them fallible").** Rationale: making `CreditLedger`/`SpentSet`/`IssueIdempotencyCache` fallible ripples across the merged, green #2 (traits + in-memory impls + all their tests + `service.rs` + `loop_e2e.rs`) for a benefit — surfacing storage IO errors — that is achievable more safely without touching #2. `lluma-broker` ships `RedbLedger`/`RedbSpentSet` implementing the EXISTING infallible traits; on a (rare, post-open) redb error they fail **closed** in the security-safe direction — `SpentSet::insert → AlreadySpent` (refuse redeem, never permit a double-spend), `debit → InsufficientCredits` (refuse issuance), `balance → 0` — and log the error via `tracing` (no client leak, L8). Idem cache stays the in-memory #2 impl for now (a retry cache, not durability-critical); durable idem + TTL is a #4 follow-up. Net: durable spent-set + ledger (the crown jewel closing #2's restart-respend hole) with **zero changes to merged #2**.
